...
const ProductCart = ({ data }: ProductCartProps) => {
  // product info
  const { title, image, rating, price, category, id } = data;
  ...
  return (
    ...
    >
      <Link to={`product/${id}`}>
        <CardActionArea>
          <CardMedia className="overflow-hidden relative bg-yellow w-full h-[300px]">
            <LazyLoadImage
              alt={title}
              src={image}
              effect="blur"
          ...
          <CardContent component="div" className="p-2 flex flex-col">
